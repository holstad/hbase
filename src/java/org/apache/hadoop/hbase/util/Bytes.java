...
public class Bytes {
   ...
   * Byte array comparator class.
   */
  public static class ByteArrayInvComparator implements RawComparator<byte []> {
    public ByteArrayInvComparator() {
      super();
    }
    @Override
    public int compare(byte [] left, byte [] right) {
      return compareTo(right, left);
    }
    @Override
    public int compare(byte [] b1, int s1, int l1, byte [] b2, int s2, int l2) {
      return compareTo(b2, s2, l2, b1, s1, l1);
    }
  }
   ...
   * Pass this to TreeMaps where byte [] are keys.
   */
  public static Comparator<byte []> BYTES_INV_COMPARATOR =
    new ByteArrayInvComparator();
